...
from Diplom.updating_diplom.optimize_diplom.image_updating_dp import Update, show_img
from Diplom.updating_diplom.optimize_diplom.rotate_dp import Rotate
from Diplom.updating_diplom.optimize_diplom.searchBD_diplom.correct_word_dp import *
from Diplom.updating_diplom.optimize_diplom.searchBD_diplom.text_detection_dp import Tesseract
#from Optimize.SearchBD.text_detection import *
from Diplom.updating_diplom.optimize_diplom.searchBD_diplom.choose_rightBD_dp import FindWord

from Diplom.updating_diplom.get_all_module import GetFunctions
...
class Diplom:
    ...
    def __init__(self):
        ...
        self.module_functions = GetFunctions()
        #self.paddle_model = Paddle()
    ...
    def __symbol_blocks(self, left_block, right_block, img):
        ...
        for i in range(len(all_blocks)):
            ...
            for ind, block in enumerate(blocks):
                    ...
                    if i == 1:
                        if flag == 0:
                            if self.rightBD.useful_functions.check_digit(words[ind], i) and ind>10 and len(words[ind]) > 5:
                                parts += left_max_blocks
                                info1.append(parts.copy())
                                parts.clear()
                                index_word = self.module_functions.check_for_letter(words[ind])
                                word = self.module_functions.text_recognition(block_img, self.tes_model)
                                #word2 = self.text_recognition(block_img, self.paddle_model)
                                if index_word == -1:
                    ...
                    elif flag[1] == 1 and i == 0 and self.module_functions.compare(block, self.doctr_model.max_block_left):
                    ...
                    elif self.module_functions.compare(block, self.doctr_model.prev_max_block_left) and i==0:
                        ...
                        if self.module_functions.check_digit(words[ind]) and count_digit_left < 2:
                            if len(words[ind]) < 3:
                                continue
                            count_digit_left +=1
                            ...
                            if count_digit_left == 2:
                                ...
                                prev_y = block[1]
                            continue
                        elif self.module_functions.check_digit(words[ind]) and count_digit_left >=2:
                            ...
                            if blocks[ind-1][0]<=left_gap<=blocks[ind-1][2]:
                                ...
                                word = self.module_functions.text_recognition(prev_block, self.tes_model)
                            ...
                            if len(blocks) != ind+1:
                                if blocks[ind+1][0]<=right_gap<=blocks[ind+1][2]:
                                    ...
                                    word = self.module_functions.text_recognition(next_block, self.tes_model)
                                    for j in range(len(parts)):
                                        if words[ind +1] == parts[j][0]:
                                            break
                                    else:
        ...
        for i in range(len(info)):
            ...
            if i == 0:
                ...
                try:
                    ...
                    cities = ['']
                try:
                    best_university, best_city = self.rightBD.findUniversity(new_word, cities)
                except:
                    best_university, best_city = '', ''
                self.result['Место:'] = best_city
                self.result['Университет:'] = best_university
            if i == 1:
                word = info[i][-1]
                try:
        ...
        try:
            for i in range(len(info)):
                ...
                if i == 1:
                    ...
                    if not self.module_functions.check_digit(spec_number):
                    ...
                    if spec_number != '' and confidence:
                        ...
                        self.result['Номер специальности:'] = info[i][0][1]
                ...
                if i == 3:
                    if self.result['Степень:'] == 'специалитет':
                        ...
                        try:
                            quality = self.rightBD.findQuality(cmp_speciality, special_number, confidence)
                        except:
                            quality = ''
