...
from Diplom.updating_diplom.optimize_diplom.searchBD_diplom.words_modules import BDFunctions, Get_BD

#from sqlalchemy.orm import sessionmaker
from Diplom.updating_diplom.optimize_diplom.searchBD_diplom.data_diplom.models_dp import Bachelor, Magistr, Speciality, University, Namessurnames, Families
from Diplom.updating_diplom.optimize_diplom.searchBD_diplom.data_diplom.to_bd_dp import get_item
...
class FindWord:
    ...
    def __init__(self):
        ...
        self.useful_functions = BDFunctions()
        ...
        #self.universities = np.array([universities['fullname'], universities['adress']])
        universities_name = np.array(universities['fullname'])
        ...
        for i in range(self.cities_university.shape[0]):
            if self.cities_university[i] not in self.universities_by_cities.keys():
                self.universities_by_cities[self.cities_university[i]] = [universities_name[i]]
            else:
                self.universities_by_cities[self.cities_university[i]].append(universities_name[i])

        #universe = []
        #for key in self.universities_by_cities.keys():
        #    for value in self.universities_by_cities.values():
        #        universe.append([key, value])
        #self.universities = np.array(universe)
    ...
    def __check_best_word_three(self, data, word_cmp):
        '''
        Функция нахождения наиболее схожего слова из БД
        data: список данных
        word_cmp: Тестируемое слово
    ...
    def __check_best_university_three(self, cities, word_cmp):
        '''
        Функция нахождения наиболее схожего слова из БД
        data: список данных
        word_cmp: Тестируемое слово
        ...
        best_city = cities[0]
        best_rate = 0
        best_university = self.universities_by_cities[best_city][0]
        for city in cities:
            for university in self.universities_by_cities[city]:
                rate = self.cmp_rates(word_cmp, university)
                if rate > best_rate:
                    best_rate = rate
        ...
        #best_word = data[0]
            ...
            #    best_word = word
        #best_three = [word[0] for word in best_three if word[0] is not None]
        return best_university, best_city
    ...
    def findCity(self, word, possible_cities):
        ...
        if city == '':
            ...
            all_words = self.useful_functions.limit_words(all_words)

            city = lv.quickmedian(all_words) #all_words
            city = city.lower().capitalize()
            city = 'г. '+city
        best_cities = self.__check_best_word_three(self.cities_university, city)
        return best_cities
    ...
    def findUniversity(self, string, city):
        ...
        if city[0] == '':
            ...
            #full_data.clear()
        if all_cities == True:
            best_university, best_city = self.__check_best_university_three(self.cities_university, string)
        else:
            best_university, best_city = self.__check_best_university_three(city, string)
        return best_university, best_city

    def eng_gramma(self, word):
        # print('Filter word:', word)
        is_digit = self.useful_functions.check_digit(word)
        count_sogl = 0
        count_gl = 0
        #copy_word = word.lower()
        for w in word:
            if w in self.eng_sogl:
                count_sogl+=1
                count_gl = 0
            elif w in self.eng_gl:
                count_gl+=1
                count_sogl=0
            else:
                count_gl = 0
    ...
    def filt_words(self, words):
        ...
        words = st.split(' ')
        words = list(filter(self.useful_functions.filt_names, words))
    ...
    def findRegNumbDataGiven(self, reg_number, data):
        ...
        while i < data_length:
            ...
            d1 = self.useful_functions.checkfordigits(data[i][0])
            d2 = self.useful_functions.checkfordigits(data[i][1])
            '''if i-1 > -1:
        ...
        for ind, word in enumerate(data):
            if self.useful_functions.check_digit(word[0]):
                if ind ==2:
                    if len(word[0]) > 4:
                    ...
                    date.append(word[0])
            elif self.useful_functions.check_digit(word[1]):
                date.append(word[1])
            elif word[1] != '':
                cmp_month = [month.lower().capitalize() for month in cmp_month]
                month, _ = self.__check_best_word(cmp_month, word[1].lower().capitalize())
                month = month.lower()
                date.append(month)
                ...
                months = self.best_month[0].lower()#[month[0].lower() for month in self.best_month if month[0] is not None]
        ...
        if date[1] == '':
            date[1] = 'января'
        #if type(date[1]) != list:
        #    date[1] = 'января'#['января', 'июня', 'ноября']
        #for month in date[1]:
        full_dates = ' '.join(date)
    ...
    def findFIO(self, fio):
        ...
        '''
        fio_bd = []
        poss_word, rate = None, None
        for i in range(len(fio)):
            word = fio[i][1]
            if word == '':
                word = fio[i][2]
                word = self.eng_gramma(word)
                if word == None:
                    fio_bd.append('')
                    continue
                word = word.upper()
                #self.filt_words()
                all_words = self.check_before_translate(word, size = min(len(word), 10))
            ...
            if None in self.begin_fio_bd[word[0]]:
                if i == 0:
                    poss_word, rate = self.__check_best_word(self.families, word)
                if i == 1:
                    poss_word, rate = self.__check_best_word(self.names, word)
                if i == 2:
                    poss_word, rate = self.__check_best_word(self.surnames, word)
            else:
                if i == 0:
                    poss_word, rate = self.__check_best_word(self.families[self.begin_fio_bd[word[0]][i][0]: self.begin_fio_bd[word[0]][i][1]], word)
                if i == 1:
                    poss_word, rate = self.__check_best_word(self.names[self.begin_fio_bd[word[0]][i][0]: self.begin_fio_bd[word[0]][i][1]], word)
                if i == 2:
                    poss_word, rate = self.__check_best_word(self.surnames[self.begin_fio_bd[word[0]][i][0]: self.begin_fio_bd[word[0]][i][1]], word)
            word = self.useful_functions.check_rate(word, poss_word, rate)
            if self.filt_words([word]) == [] or word == '' or word == None:
                word = poss_word
            fio_bd.append(word)
        return fio_bd
    ...
    def findSpecNumber(self, number):
        ...
        num_2_spec = None
        if number[0] != '':
            number[0] = self.useful_functions.clean_number(number[0])
        ...
        if number[1] != '':
            num = self.useful_functions.clean_number(number[1])
            number[1] = num
    ...
    def findSpecialName(self, name, level):
        ...
        if word1 != '':
            all_names,_ = self.__check_best_word(data, word1)
        else:
            words = []
            for word in name:
            ...
            all_names, _ = self.__check_best_word(data, poss_word)
        return all_names
    ...
    def findCmpLevel(self, level, cmp_level):
        ...
        level, _ = self.__check_best_word(specs, word_cmp)
        #print(level)
        #level = self.cmp_lv_date(level, specs)
        return level[0].lower()


    def findQuality(self, word, special_number, confidence):
        '''
        ...
        if special_number !='' and confidence:
            poss_qualities = self.spec_quality[np.where(self.spec_number == special_number)].copy()
            poss_qualities = poss_qualities.split('.')
            best_word, best_rate = self.__check_best_word(poss_qualities, word_cmp)
        else:
            #best_word, best_rate = self.__check_best_word(self.spec_quality, word_cmp)
            word_cmp = word_cmp.lower().capitalize()
        ...
        word = self.useful_functions.check_rate(word_cmp, best_word, best_rate, min_rate = 0.4)
        if self.filt_words([word]) == [] or word == '' or word == None:
            word = word_cmp
