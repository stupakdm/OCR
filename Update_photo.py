...
class Update:
    ...
    def find_max_ext(contours, fl = 0):
        ...
        if fl == 0:
            ...
            return (max_extLeft[0]+abs(max_extLeft[0] - max_extRight[0])//2, max_extTop[1]+abs(max_extTop[1] - max_extBot[1])//2)
    ...
    def draw_contours(self, image, orig_image):
        ...
        #print(contours[-2])
        #contours = contours[-2]
        img = cv2.drawContours(orig_image, contours, contourIdx=-1, color=(0,0,0), thickness=cv2.FILLED, hierarchy=hierarchy)
        return img
        #for c in contours:
