...
from modify_passport.image_updating_passport.updating_image_ps import Update


class New_Rotate:
    '''
    Класс для поврота изображения
    '''
    def __init__(self):
    ...
    def find_biggest_rect(self, image):
        ...
        redImage = self.updating.erosion(redImage, k=(10,2), board='ISOLATED')
        ...
        Rect, flag = self.updating.find_rect(redModifiedImage, 1)
        return Rect, flag

    def cvt_image(self, image):
        hsv = self.updating.change_hsv(image, hue=-30, value=-30, satur=-30)
        return self.updating.extract_red_contours(hsv)
    ...
    def another_rotate(self, orig_image, image):
        ...
        gray = self.updating.black_white(image)
        show_img(gray, 'GRAY!!!')
        # sobel = show_img(Update.sobel_operator(self, orig_image), 'Try_sobel')
        try:
            rect, flag = self.updating.find_rect(gray, 2)
        except:
            return orig_image
        flag = 1
